"""Importer tutorial visual UC-P11 (Sprint 3)."""
...
def importar():
    print(f"=== importar_tutorial_uc_p11 ===")
    db = get_db()
    try:
        ...
        if not ct:
            ...
            ct = CasoDeTeste(id=_uuid(), caso_de_uso_id=uc.id, ct_id=CT_ID,
                descricao=f"FP UC-P11", tipo="Positivo", categoria="Cenário",
                trilha_sugerida="visual", variacao_tutorial="fp", ativo=1)
            db.add(ct)
            db.flush()

        tut = carregar_tutorial(UC_ID, VARIACAO, "piloto-ucp11")
