"""Seed Sprint 5 (UC-FU01..03 + UC-AT01..03 + UC-CT01..10 + UC-CR01..03 + UC-CRM01..07 = 26 UCs)."""
...
def main():
    db = get_db()
    try:
        sprint = db.query(Sprint).filter_by(numero=5).first()
        if not sprint:
            sprint = Sprint(numero=5, nome="Sprint 5 — Followup, Atas, Execucao, Contratado x Realizado e CRM",
                            projeto_id="00000000-0000-0000-0000-000000000001")
            db.add(sprint); db.flush()
            print(f"Sprint 5 criada id={sprint.id}")

        ucs_lista = (
            [f"UC-FU{n:02d}" for n in range(1, 4)] +
            [f"UC-AT{n:02d}" for n in range(1, 4)] +
            [f"UC-CT{n:02d}" for n in range(1, 11)] +
            [f"UC-CR{n:02d}" for n in range(1, 4)] +
            [f"UC-CRM{n:02d}" for n in range(1, 8)]
        )
        n_uc = 0; n_ct = 0
        ...
        db.commit()
        print(f"\n=== OK ===")
        print(f"  {n_uc} UCs, {n_ct} CTs novos")
        return 0
    finally:
        db.close()
