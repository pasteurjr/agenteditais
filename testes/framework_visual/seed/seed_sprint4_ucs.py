"""Seed Sprint 4 (UC-I01..I05 + UC-RE01..RE06) com CTs esqueleto."""
...
def main():
    db = get_db()
    try:
        sprint = db.query(Sprint).filter_by(numero=4).first()
        if not sprint:
            sprint = Sprint(numero=4, nome="Sprint 4 — Recursos e Impugnacoes",
                            projeto_id="00000000-0000-0000-0000-000000000001")
            db.add(sprint); db.flush()
            print(f"Sprint 4 criada id={sprint.id}")

        ucs_lista = [f"UC-I{n:02d}" for n in range(1, 6)] + [f"UC-RE{n:02d}" for n in range(1, 7)]
        n_uc = 0; n_ct = 0
