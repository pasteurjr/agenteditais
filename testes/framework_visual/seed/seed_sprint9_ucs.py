"""Seed Sprint 9 (UC-LA01..06 + UC-SC01..05 + UC-HC01 = 12 UCs)."""
...
def _extrair_nome(c, uc_id):
    m = re.search(r"^nome:\s*['\"](.+?)['\"]", c, re.MULTILINE)
    if m: return m.group(1).strip()
    m = re.search(rf"^#\s+{re.escape(uc_id)}\s*[—-]\s*(.+?)$", c, re.MULTILINE)
    return m.group(1).strip() if m else uc_id


def main():
    db = get_db()
    try:
        sprint = db.query(Sprint).filter_by(numero=9).first()
        if not sprint:
            sprint = Sprint(numero=9, nome="Sprint 9 — Lances/Sala Virtual + Scores + Health Check",
                            projeto_id="00000000-0000-0000-0000-000000000001")
            db.add(sprint); db.flush()
            print(f"Sprint 9 criada id={sprint.id}")
        ucs_lista = ([f"UC-LA{n:02d}" for n in range(1, 7)] +
                     [f"UC-SC{n:02d}" for n in range(1, 6)] +
                     ["UC-HC01"])
        n_uc = 0; n_ct = 0
        for uc_id in ucs_lista:
            path = _PROJECT / "testes" / "casos_de_uso" / f"{uc_id}.md"
            if not path.exists(): print(f"  [warn] {path} nao existe"); continue
            ...
            if existing:
                existing.nome = nome; existing.doc_origem = str(path.relative_to(_PROJECT))
                existing.conteudo_md = cont; uc_pk = existing.id
            else:
