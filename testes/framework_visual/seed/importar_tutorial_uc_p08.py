"""Importer tutorial visual UC-P08 (Sprint 3)."""
...
def importar():
    print(f"=== importar_tutorial_uc_p08 ===")
    db = get_db()
    try:
        ...
        if not ct:
            ...
            ct = CasoDeTeste(id=_uuid(), caso_de_uso_id=uc.id, ct_id=CT_ID,
                descricao=f"FP UC-P08", tipo="Positivo", categoria="Cenário",
                trilha_sugerida="visual", variacao_tutorial="fp", ativo=1)
            db.add(ct)
            db.flush()

        tut = carregar_tutorial(UC_ID, VARIACAO, "piloto-ucp08")
