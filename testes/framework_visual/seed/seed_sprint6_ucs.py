"""Seed Sprint 6 (UC-FL01..05 + UC-MO01..06 + UC-AU01..03 + UC-SM01..03 = 17 UCs)."""
...
def main():
    db = get_db()
    try:
        sprint = db.query(Sprint).filter_by(numero=6).first()
        if not sprint:
            sprint = Sprint(numero=6, nome="Sprint 6 — Alertas, Monitoramentos, Auditoria, SMTP",
                            projeto_id="00000000-0000-0000-0000-000000000001")
            db.add(sprint); db.flush()
            print(f"Sprint 6 criada id={sprint.id}")

        ucs_lista = (
            [f"UC-FL{n:02d}" for n in range(1, 6)] +
            [f"UC-MO{n:02d}" for n in range(1, 7)] +
            [f"UC-AU{n:02d}" for n in range(1, 4)] +
            [f"UC-SM{n:02d}" for n in range(1, 4)]
        )
        n_uc = 0; n_ct = 0
