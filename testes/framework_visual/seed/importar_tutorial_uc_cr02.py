"""Importer tutorial UC-CR02 (Sprint 5)."""
    ...
    try:
        ...
        if not ct:
            ...
            ct = CasoDeTeste(id=_uuid(), caso_de_uso_id=uc.id, ct_id=CT_ID,
                descricao=f"FP UC-CR02", tipo="Positivo", categoria="Cenário",
                trilha_sugerida="visual", variacao_tutorial="fp", ativo=1)
            db.add(ct); db.flush()
        tut = carregar_tutorial(UC_ID, "fp", "piloto-uccr02")
