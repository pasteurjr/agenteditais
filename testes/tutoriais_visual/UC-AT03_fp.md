...
# UC-AT03 — Listar Minhas Atas (Sprint 5 V3 PROFUNDO direto via API)

> **Estrategia:** chamada direta GET /api/atas/minhas + valida resposta
...
acao:
  sequencia:
    ...
    - tipo: wait
      valor_literal: 200
validacao_ref: "testes/casos_de_teste/UC-AT03_visual_fp.yaml#passo_00_setup"
```
...
acao:
  sequencia:
    - tipo: evaluate
      valor_literal: |
        async () => {
          ...
          const r = await fetch('/api/atas/minhas', { headers: { Authorization: `Bearer ${token}` } });
          ...
          if (!r.ok) throw new Error(`GET /api/atas/minhas ${r.status}`);
    ...
    - tipo: wait
      valor_literal: 5000
validacao_ref: "testes/casos_de_teste/UC-AT03_visual_fp.yaml#passo_01_chamar_endpoint"
```
