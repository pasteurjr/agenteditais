...
export const api = {
  ...
  pausarTeste:   (id, runId)   => req(`/api/testes/${id}/pausar${runId ? `?run_id=${runId}` : ''}`, {method: "POST"}),
  retomarTeste:  (id, runId)   => req(`/api/testes/${id}/retomar${runId ? `?run_id=${runId}` : ''}`, {method: "POST"}),
  reiniciarTeste:(id)          => req(`/api/testes/${id}/reiniciar`, {method: "POST"}),
  cancelarTeste: (id, runId)   => req(`/api/testes/${id}/cancelar${runId ? `?run_id=${runId}` : ''}`, {method: "POST"}),
  adicionarUcs:  (id, uc_ids, runId) => req(`/api/testes/${id}/adicionar-ucs${runId ? `?run_id=${runId}` : ''}`, {method: "POST", body: JSON.stringify({uc_ids})}),
