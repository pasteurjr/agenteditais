...
Esta versão V4 documenta as Regras de Negócio (RNs) já enforçadas no backend. Por padrão estão em modo **warn-only** (`ENFORCE_RN_VALIDATORS=false`) — logam warning mas não bloqueiam. Ativar com `ENFORCE_RN_VALIDATORS=true`.

**Total de RNs únicas mapeadas para os UCs de CAPTACAO/VALIDACAO (Sprint 2):** 45 RNs (RN-043 a RN-087), distribuídas entre 11 UCs (UC-CV01 a UC-CV04, UC-CV07 a UC-CV13). Destas, 13 são marcadas `[FALTANTE→V4]` (implementadas nesta versão): RN-078, RN-079, RN-080, RN-081, RN-082, RN-083, RN-084, RN-085, RN-086, RN-087. Os UCs UC-CV05 e UC-CV06 não possuem RNs mapeadas em `/tmp/rns_CAPTACAO.md`.

| RN | Descrição | UC afetado | Arquivo backend |
|---|---|---|---|
| RN-077 | Termo de busca ≥3 caracteres | UC de busca por termo | `backend/app.py` |
| RN-078 | Filtro UF só aceita UFs IBGE válidas (27 estados + DF) | UC de busca com filtro UF | `backend/rn_validators.py::validar_uf` |
| RN-082 | Transição de estado Edital: novo→analisando→participando→proposta_enviada→em_pregao→vencedor/perdedor | UCs de gestão de edital (salvar, analisar, decidir) | `backend/rn_estados.py::EDITAL_TRANSITIONS` |
| RN-080 | Versionamento de decisão GO/NO-GO (toda mudança em estrategias-editais é logada em AuditoriaLog com snapshot antes/depois) | UC de decisão GO/NO-GO | `backend/rn_audit.py::log_transicao` + `crud_routes._AUDITED_TABLES` |
| RN-083 | Escopo de chat limitado ao edital aberto (impede tool em chat consultar outro edital) | UC de chat IA em validação | `backend/rn_deepseek.py::validar_scope_edital` |
| RN-084 | Cooldown de 60s entre chamadas DeepSeek por empresa (retorna HTTP 429 se ativado) | UC de análise IA, chat, geração de proposta | `backend/rn_deepseek.py::check_cooldown` (wired em `/api/chat`) |
| RN-132 | Audit log de invocações de ferramentas DeepSeek (tool name, input hash, duração) | Todos os UCs com tools IA | `backend/rn_audit.py::audited_tool` |
| RN-037 | Audit log universal (editais, propostas, estratégias são logadas em AuditoriaLog) | UCs de CRUD de edital, proposta, estratégia | `backend/rn_audit.py::log_transicao` |
