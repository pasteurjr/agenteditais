...
## UC-F13 V8 — Gerir e consultar classificação Área/Classe/Subclasse

> **V8 (29/04/2026):** UC-F13 reescrito. Agora **CRIA** as 2 áreas + 3 classes + 3 subclasses via 3 CRUDs (`areas-produto`, `classes-produto-v2`, `subclasses-produto`). A empresa RP3X nasce sem hierarquia — você cria toda a estrutura aqui antes de qualquer outro UC que dependa dela. Ver `tutorialsprint1-2 V2.md` seção UC-F13 para passo-a-passo humano.

### Hierarquia completa a CRIAR (CT-F13-FP)

#### Área 1: Equipamentos Médico-Hospitalares

| Nível | Nome | NCM |
|---|---|---|
| Área | `Equipamentos Médico-Hospitalares` | — |
| Classe | `Monitoração` | — |
| Subclasse | `Monitor Multiparamétrico` | `9018.19.90` |

#### Área 2: Diagnóstico in Vitro e Laboratório (compartilhada por 2 classes)

| Nível | Nome | NCM |
|---|---|---|
| Área | `Diagnóstico in Vitro e Laboratório` | — |
| Classe | `Reagentes Bioquímicos` | — |
| Subclasse | `Reagente para Glicose` | `3822.19.90` |

| Nível | Nome | NCM |
|---|---|---|
| Área | `Diagnóstico in Vitro e Laboratório` (mesma — não criar de novo) | — |
| Classe | `Reagentes e Kits Diagnósticos` | — |
| Subclasse | `Kit de Hematologia` | `3822.19.90` |

**Total a criar:** 2 áreas, 3 classes, 3 subclasses.

### Endpoints chamados pelo CRUD (referência técnica)

- `POST /api/crud/areas-produto` (payload `{nome}`)
- `POST /api/crud/classes-produto-v2` (payload `{nome, area_id}`)
- `POST /api/crud/subclasses-produto` (payload `{nome, classe_id, ncm}`)

### Visualização final (PortfolioPage → aba Classificação)

Após criar tudo, navegar até PortfolioPage → aba **Classificação** e expandir cada área/classe para ver as 3 subclasses listadas com os NCMs corretos.

| Nível visualizado | Quantidade esperada |
|---|---|
| Áreas listadas | 2 |
| Classes listadas | 3 |
| Subclasses listadas | 3 |
| NCMs distintos | 2 (`9018.19.90` e `3822.19.90`) |
