...
> **V8 do UC (29/04/2026):** Reescrito. Antes era apenas selecionar uma hierarquia preexistente. Agora **CRIA** Área → Classe → Subclasse via 3 CRUDs (`empresa_scoped=True` — cada empresa tem sua própria hierarquia) e depois consulta a árvore consolidada.

**Páginas:**
- `crud:areas-produto` (Cadastros → Áreas de Produto)
- `crud:classes-produto-v2` (Cadastros → Classes de Produto)
- `crud:subclasses-produto` (Cadastros → Subclasses de Produto)
- `PortfolioPage` (`/app/portfolio` aba Classificação)

**Pré-condições:**
- UC-F01 + UC-F18 concluídos (empresa criada e vinculada ao usuário corrente)
- Empresa selecionada como ativa
- Sem hierarquia prévia (empresa nova) — tutorial é idempotente: pula criação se item já existe

---

### Hierarquias canônicas a criar
...
Diagnóstico in Vitro e Laboratório
├── Reagentes Bioquímicos
│   └── Reagente para Glicose (NCM 3822.19.90)
└── Reagentes e Kits Diagnósticos
    └── Kit de Hematologia (NCM 3822.19.90)
```
...
### Sequência de Automação Playwright

```typescript
// ===== PARTE 1 — Áreas =====
// Sidebar: expandir Cadastros, clicar Áreas de Produto
await page.click('button.nav-section-header:has-text("Cadastros")');
await page.click('.nav-item-label:text-is("Áreas de Produto")');
await expect(page.locator('h1, h2').filter({ hasText: /Áreas de Produto/ })).toBeVisible();

// Criar Área 1: Equipamentos Médico-Hospitalares
await page.click('button:has-text("Novo")');
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Equipamentos Médico-Hospitalares');
await page.click('button:has-text("Salvar")');
await expect(page.locator('.toast, [role="alert"]')).toContainText(/salvo|sucesso/i);

// Criar Área 2: Diagnóstico in Vitro e Laboratório
await page.click('button:has-text("Novo")');
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Diagnóstico in Vitro e Laboratório');
await page.click('button:has-text("Salvar")');

// ===== PARTE 2 — Classes =====
await page.click('.nav-item-label:text-is("Classes de Produto")');
await expect(page.locator('h1, h2').filter({ hasText: /Classes de Produto/ })).toBeVisible();

// Classe 1: Monitoração (vinculada a Equipamentos)
await page.click('button:has-text("Novo")');
await page.selectOption('select[name="area_id"], label:has-text("Área") + select', { label: 'Equipamentos Médico-Hospitalares' });
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Monitoração');
await page.click('button:has-text("Salvar")');

// Classe 2: Reagentes Bioquímicos (vinculada a Diagnóstico)
await page.click('button:has-text("Novo")');
await page.selectOption('select[name="area_id"], label:has-text("Área") + select', { label: 'Diagnóstico in Vitro e Laboratório' });
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Reagentes Bioquímicos');
await page.click('button:has-text("Salvar")');

// Classe 3: Reagentes e Kits Diagnósticos (vinculada a Diagnóstico)
await page.click('button:has-text("Novo")');
await page.selectOption('select[name="area_id"], label:has-text("Área") + select', { label: 'Diagnóstico in Vitro e Laboratório' });
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Reagentes e Kits Diagnósticos');
await page.click('button:has-text("Salvar")');

// ===== PARTE 3 — Subclasses (com NCM) =====
await page.click('.nav-item-label:text-is("Subclasses de Produto")');
await expect(page.locator('h1, h2').filter({ hasText: /Subclasses de Produto/ })).toBeVisible();

// Subclasse 1: Monitor Multiparamétrico (Monitoração / NCM 9018.19.90)
await page.click('button:has-text("Novo")');
await page.selectOption('select[name="classe_id"], label:has-text("Classe") + select', { label: 'Monitoração' });
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Monitor Multiparamétrico');
await page.locator('input[name="ncm"], label:has-text("NCM") + input').fill('9018.19.90');
await page.click('button:has-text("Salvar")');

// Subclasse 2: Reagente para Glicose (Reagentes Bioquímicos / NCM 3822.19.90)
await page.click('button:has-text("Novo")');
await page.selectOption('select[name="classe_id"], label:has-text("Classe") + select', { label: 'Reagentes Bioquímicos' });
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Reagente para Glicose');
await page.locator('input[name="ncm"], label:has-text("NCM") + input').fill('3822.19.90');
await page.click('button:has-text("Salvar")');

// Subclasse 3: Kit de Hematologia (Reagentes e Kits / NCM 3822.19.90)
await page.click('button:has-text("Novo")');
await page.selectOption('select[name="classe_id"], label:has-text("Classe") + select', { label: 'Reagentes e Kits Diagnósticos' });
await page.locator('input[name="nome"], label:has-text("Nome") + input').fill('Kit de Hematologia');
await page.locator('input[name="ncm"], label:has-text("NCM") + input').fill('3822.19.90');
await page.click('button:has-text("Salvar")');

// ===== PARTE 4 — Visualizar árvore consolidada =====
await page.click('button.nav-section-header:has-text("Configurações")');
await page.click('.nav-item-label:text-is("Portfolio")');
await page.click('text=Classificação');
// Áreas listadas
await expect(page.getByText('Equipamentos Médico-Hospitalares')).toBeVisible();
await expect(page.getByText('Diagnóstico in Vitro e Laboratório')).toBeVisible();
// Expandir e conferir
await page.click('text=Equipamentos Médico-Hospitalares');
await expect(page.getByText('Monitoração')).toBeVisible();
await page.click('text=Monitoração');
await expect(page.getByText('Monitor Multiparamétrico')).toBeVisible();
await expect(page.getByText('9018.19.90')).toBeVisible();
```

### Passos detalhados (resumo)

| Parte | Ação | Quantidade |
|---|---|---|
| 1 — Áreas | POST `/api/crud/areas-produto` | 2 áreas |
| 2 — Classes | POST `/api/crud/classes-produto-v2` (com `area_id`) | 3 classes |
| 3 — Subclasses | POST `/api/crud/subclasses-produto` (com `classe_id` + `ncm`) | 3 subclasses |
| 4 — Visualização | GET PortfolioPage aba Classificação | 1 verificação |

### Verificações finais (asserções)
...
await expect(page.getByText('Monitor Multiparamétrico')).toBeVisible();
await expect(page.getByText('Diagnóstico in Vitro e Laboratório')).toBeVisible();
await expect(page.getByText('Reagentes Bioquímicos')).toBeVisible();
await expect(page.getByText('Reagente para Glicose')).toBeVisible();
await expect(page.getByText('Reagentes e Kits Diagnósticos')).toBeVisible();
await expect(page.getByText('Kit de Hematologia')).toBeVisible();
await expect(page.getByText('9018.19.90')).toBeVisible();
await expect(page.getByText('3822.19.90')).toBeVisible();
```

> **Nota para execução manual:** UC-F13 V8 requer ~12-18 minutos. UCs subsequentes (F02, F06, F08, Sprint 2) só funcionam depois.
