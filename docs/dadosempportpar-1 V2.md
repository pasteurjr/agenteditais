...
> **V8 (29/04/2026):** UC-F13 reescrito. Agora **CRIA** a hierarquia (3 áreas, 3 classes, 3 subclasses) via 3 CRUDs, antes de visualizar. Ver `tutorialsprint1-1 V2.md` seção UC-F13.
...
#### Área 2: Diagnóstico in Vitro e Laboratório
...
| Nível | Nome | NCM |
|---|---|---|
| Área | `Diagnóstico in Vitro e Laboratório` (mesma) | — |
| Classe | `Reagentes e Kits Diagnósticos` | — |
| Subclasse | `Kit de Hematologia` | `3822.19.90` |

**Total a criar:** 2 áreas, 3 classes, 3 subclasses.

### Endpoints chamados (referência)

- `POST /api/crud/areas-produto` (`{nome}`)
- `POST /api/crud/classes-produto-v2` (`{nome, area_id}`)
- `POST /api/crud/subclasses-produto` (`{nome, classe_id, ncm}`)

### Visualização final (PortfolioPage > aba Classificação)

| Verificação | Valor esperado |
|---|---|
| Áreas listadas | 2 (Equipamentos Médico-Hospitalares + Diagnóstico in Vitro) |
| Classes listadas | 3 (Monitoração, Reagentes Bioquímicos, Reagentes e Kits Diagnósticos) |
| Subclasses listadas | 3 (Monitor Multiparamétrico, Reagente para Glicose, Kit de Hematologia) |
| NCMs visíveis | 9018.19.90 e 3822.19.90 |
...
| Filtro Inteligente (tags) | "Monitoração", "Diagnóstico in Vitro" |
| Classificação Automática | 3 classes |
