...
> **CHANGELOG V8 (14/05/2026) — UC-F12 metadados editáveis manualmente**
>
> - **UC-F12 Reescrito:** Era "Reprocessar metadados". Agora é "**Gerir metadados** (consultar, editar manualmente OU reprocessar via IA)".
> - **Novos botões na UI:** "✏ Editar metadados manualmente" / "✓ Salvar metadados" / "Cancelar".
> - **Sequência de eventos atualizada** com 2 caminhos: (FA-01) reprocessar via IA (default), (FA-02) editar manualmente.
> - **Backend:** sem mudança (PUT `/api/crud/produtos/<id>` aceita `catmat_codigos`, `catser_codigos`, `termos_busca` como JSON arrays).
> - **Decisão de produto:** revogada a regra "read-only" da V4 — agora o usuário pode override manual da IA quando os termos gerados não capturam corretamente o produto.
...
## [UC-F12] Gerir metadados de captacao do produto (V8 — agora editavel)
...
> **NOVIDADE V8 (14/05/2026):** Antes apenas reprocessavel via IA. Agora os 3 campos (`catmat_codigos`, `catser_codigos`, `termos_busca`) sao editaveis manualmente pelo usuario. Util quando a IA nao acerta os codigos ou quando se quer override de termos. O reprocessamento via IA continua disponivel como segunda opcao.

### Pre-condicoes
1. Produto com detalhe aberto.
2. Endpoint `reprocessarMetadados(produtoId)` disponivel para Fluxo Principal IA.
3. Endpoint `PUT /api/crud/produtos/<id>` operacional para Fluxo Alternativo Manual (aceita arrays JSON em `catmat_codigos`, `catser_codigos`, `termos_busca`).
...
### Pos-condicoes
1. CATMAT, CATSER, termos de busca e timestamp de atualizacao foram recalculados (via IA) OU sobrescritos manualmente (via PUT).
2. Detalhe do produto e recarregado.
3. Em caso de edicao manual, os 3 arrays JSON sao persistidos diretamente no registro do produto.

### Botoes e acoes observadas
- toggle `Metadados de Captacao`
- `Editar metadados manualmente` (NOVO V8)
- `Salvar metadados` (NOVO V8)
- `Cancelar` (NOVO V8)
- `Reprocessar Metadados`

### Sequencia de eventos (Fluxo Principal — Reprocessamento via IA)
...
2. Sistema exibe: [Tag: codigos CATMAT] com [Badge: "IA"], [Texto: descricoes CATMAT], [Tag: codigos CATSER], [Tag: termos de busca semanticos] (verde) e [Texto: "Ultima Atualizacao"]. Acima dos campos aparece o [Botao: "Editar metadados manualmente"].
3. Usuario clica no [Botao: "Reprocessar Metadados"] [Icone: RefreshCw] no rodape do bloco.
...
**FA-01 — Metadados ja existentes (nao reprocessa)**
1. Ao expandir toggle, metadados ja estao preenchidos.
2. Usuario opta por nao reprocessar e nao editar. Apenas consulta.

**FA-02 — Edicao manual dos metadados (NOVO V8)**
1. Apos expandir o [Toggle: "Metadados de Captacao"], o usuario clica em [Botao: "✏ Editar metadados manualmente"] no topo do bloco.
2. Sistema substitui as Tags read-only por [Input: text] editaveis, um para cada campo:
   - [Input: "Codigos CATMAT"] aceita valores separados por virgula (ex: `495268, 495269`).
   - [Input: "Codigos CATSER"] aceita valores separados por virgula (ex: `27502, 27510`).
   - [Input: "Termos de Busca Semanticos"] aceita valores separados por virgula (ex: `hemograma, hematologia, CBC`).
3. Sistema exibe banner amarelo "Modo edicao manual — separe multiplos valores por virgula. Salvar substitui os valores da IA."
4. Usuario edita os campos e clica em [Botao: "✓ Salvar metadados"] (verde).
5. Sistema envia `PUT /api/crud/produtos/<id>` com payload:
   ```json
   {
     "catmat_codigos": ["495268", "495269"],
     "catser_codigos": ["27502", "27510"],
     "termos_busca": ["hemograma", "hematologia", "CBC"]
   }
   ```
6. Apos 200 OK, sistema executa novo `getProduto(produtoId)`, fecha o modo edicao e exibe mensagem verde "✓ Metadados salvos com sucesso" (3 segundos).
7. As Tags read-only voltam a ser exibidas com os novos valores.

**FA-03 — Cancelar edicao manual**
1. Durante o modo edicao (apos FA-02 passo 2), usuario clica em [Botao: "Cancelar"].
2. Sistema descarta as alteracoes feitas nos inputs e volta ao modo read-only com os valores originais (sem chamar a API).
...
**FE-03 — Falha ao salvar manualmente (NOVO V8)**
1. `PUT /api/crud/produtos/<id>` retorna erro (400, 500, ou rede).
2. Sistema exibe banner vermelho "✗ Erro: <mensagem>" no topo do bloco.
3. Inputs permanecem preenchidos com os valores que o usuario tentou salvar (para retry).
4. Botao "✓ Salvar metadados" volta a ficar habilitado.
...
#### Layout da Tela (V8 — modo consulta)

```
[Card: "Detalhes: {nome}"]
  ...
  [Toggle: "Metadados de Captacao"] [Icone: ChevronRight/Down] [Badge: "IA"] [ref: Passo 1]
    [Botao: "✏ Editar metadados manualmente"] (NOVO V8) [ref: FA-02 Passo 1]
    [Texto: "Codigos CATMAT"] + [Tag: codigos] [Badge: "IA"] [ref: Passo 2]
    [Texto: "Descricoes CATMAT"] (sempre read-only) [ref: Passo 2]
...
#### Layout da Tela (V8 — modo edicao manual, apos FA-02)

```
[Card: "Detalhes: {nome}"]
  ...
  [Toggle: "Metadados de Captacao"] [Badge: "IA"]
    [Banner amarelo: "Modo edicao manual — separe multiplos valores por virgula..."]
    [Texto: "Codigos CATMAT"]
    [Input: text com placeholder "ex: 495268, 495269"]
    [Texto: "Codigos CATSER"]
    [Input: text com placeholder "ex: 27502, 27510"]
    [Texto: "Termos de Busca Semanticos"]
    [Input: text com placeholder "ex: hemograma, hematologia, CBC"]
    [Botao: "✓ Salvar metadados"] (verde)  [Botao: "Cancelar"] (cinza)
    [Botao: "Reprocessar Metadados"] (roxo, ainda disponivel)
```
...
| [Tag: CATMAT / CATSER / termos] (modo consulta) | 2 |
| [Texto: "Ultima Atualizacao"] | 2 |
| [Botao: "Reprocessar Metadados"] | 3 (FP) |
| [Botao: "✏ Editar metadados manualmente"] | FA-02 Passo 1 |
| [Input: CATMAT / CATSER / Termos] (modo edicao) | FA-02 Passo 2 |
| [Banner amarelo modo edicao] | FA-02 Passo 3 |
| [Botao: "✓ Salvar metadados"] | FA-02 Passo 4-6 |
| [Botao: "Cancelar"] | FA-03 |

### Persistencia observada (V8)

| Campo | Tipo (banco) | Tipo (payload PUT) |
|---|---|---|
| `produtos.catmat_codigos` | JSON (array de strings) | `["495268","495269"]` |
| `produtos.catser_codigos` | JSON (array de strings) | `["27502","27510"]` |
| `produtos.termos_busca` | JSON (array de strings) | `["hemograma","CBC"]` |
| `produtos.catmat_descricoes` | JSON (array de strings) | NAO editavel via UI (gerado pela API SIASG durante reprocessamento) |
| `produtos.catmat_updated_at` | DateTime | atualizado em ambos os fluxos |

### Implementacao atual (V8)
**IMPLEMENTADO** — edicao manual disponivel em PortfolioPage (states `editingMetadados`, `editCatmat`, `editCatser`, `editTermos`, `savingMetadados`, `metadadosMsg`); handlers `handleAbrirEdicaoMetadados`, `handleCancelarEdicaoMetadados`, `handleSalvarMetadados`. Backend reutiliza `PUT /api/crud/produtos/<id>` ja existente.
