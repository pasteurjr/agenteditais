# Tutorial de Validação Manual — Sprint 3 — Conjunto 3 V8
# Empresa: Vita-Sense Solucoes Medicas Ltda.

**Data:** 18/05/2026 (V8)
**Dados:** dadosprecprop-3 V8.md (continuidade do tutorialsprint2-3 V8.md)
**Referência:** CASOS DE USO PRECIFICACAO E PROPOSTA V2.md
**Pré-requisito:** `tutorialsprint2-3 V8.md` executado com sucesso (empresa Vita-Sense + portfólio + parametrização + edital captado/validado)
...
> **CHANGELOG V8**
>
> Conjunto 3 V8 — encadeado com `tutorialsprint2-3 V8.md` (mesma empresa **Vita-Sense Solucoes Medicas Ltda.**). Estrutura de UCs idêntica ao molde validado (`tutorialsprint3-1.md`); identidade do conjunto adaptada (empresa / usuário / CNPJ / produtos). Reflete o estado pós-correções das 22 observações Arnaldo (Sprint 1) e melhorias de UX (cursor mãozinha, sidebar persistente, badge de 4 estados, endereço estruturado, máscaras de campo).
...
| Usuario | validaargus@valida.com.br |
| Senha | 123456 |
| Perfil | Superusuario |
| Empresa alvo | Vita-Sense Solucoes Medicas Ltda. |
...
3. Tela de selecao de empresa -> clicar "Vita-Sense Solucoes Medicas Ltda."
...
- Produtos do portfolio cadastrados na Sprint 1 (Ventilador Savina 300 e Oximetro Mindray PM-60)
...
|---|---|---|---|
| Monitor multiparametrico 12 parametros | Ventilador Pulmonar Drager Savina 300 | >= 80% | Vinculado automaticamente |
...
**O que voce vai ver na tela:** O item 1 (Monitor) deve ter um badge verde "Vinculado" com o nome do produto `Ventilador Pulmonar Drager Savina 300`. Os itens 3, 4 e 5 devem ter badge cinza "Sem match".
...
**O que o validador deve conferir:**
- Item 1 (Monitor) vinculado automaticamente ao `Ventilador Pulmonar Drager Savina 300` com match >= 80%
...
- Item 1 nao e vinculado ao Ventilador Savina 300
...
### Passo 1 — Informar custos ERP do Ventilador Savina 300

**O que fazer:** Na aba "Custos e Precos", localize a secao de custos para o produto vinculado. Preencha os custos ERP do `Ventilador Pulmonar Drager Savina 300`:

| Campo | Valor |
|---|---|
| Produto | `Ventilador Pulmonar Drager Savina 300` |
| SKU | `DRG-SVN300-BR` |
...
### Passo 1 — Calcular Preco Base do Ventilador Savina 300 (Custo + Markup)

**O que fazer:** Selecione o modo "Custo + Markup" (radio button). O sistema calcula automaticamente o Preco Base para o `Ventilador Pulmonar Drager Savina 300`:
...
**O que o validador deve conferir:**
- Ventilador Savina 300: Preco Base calculado = `R$ 22.486,40` (Custo + Markup 28%)
...
|---|---|---|---|
| Ventilador Savina 300 | `R$ 22.486,40` | `-18%` | `R$ 18.438,85` |
...
|---|---|---|---|
| Ventilador Savina 300 | `R$ 18.500,00` | `R$ 17.800,00` | `3,8%` |
...
|---|---|---|---|
| Ventilador Savina 300 | `R$ 17.800,00` | `R$ 16.500,00` | `17,5%` |
...
**O que fazer:** Na aba "Historico", selecione o produto `Ventilador Pulmonar Drager Savina 300` e o periodo `Ultimos 24 meses`. Clique em `Filtrar`.
...
| 2025-06-10 | Hospital Estadual RJ | RJ | R$ 18.900,00 | R$ 15.900,00 | Vita-Sense |
...
- Produto do portfolio cadastrado (Ventilador Savina 300)
...
| Campo | Valor |
|---|---|
| Equipamento | `Ventilador Pulmonar Drager Savina 300` |
| Numero de serie | `DRG-SVN300-2026-001` |
...
**O que o validador deve conferir:**
- Comodato cadastrado com equipamento `Ventilador Savina 300`, serie `DRG-SVN300-2026-001`, prazo `60 meses`
...
**O que fazer:** Analise as sugestoes geradas pelo pipeline para o `Ventilador Savina 300`:
...
| Secao | Conteudo |
|---|---|
| Cabecalho | Empresa (Vita-Sense), edital, lote, data de geracao |
...
- Produto `Ventilador Pulmonar Drager Savina 300` com especificacoes cadastradas no portfolio
...
| Produto ofertado | Portfolio | `Ventilador Pulmonar Drager Savina 300` |
| Fabricante | Portfolio | `Drager` |
| Modelo | Portfolio | `Savina 300` |
...
- Fabricante `Drager`, registro ANVISA `80262090001`
...
**O que fazer:** No card "Auditoria ANVISA", verifique que o produto `Ventilador Pulmonar Drager Savina 300` esta selecionado com registro `80262090001`. Clique em `Verificar Registros`.
...
| Secao | Conteudo |
|---|---|
| Capa | Empresa (Vita-Sense), edital, lote, data |
...
| UC-P10 | Comodato cadastrado serie DRG-SVN300-2026-001, amortizacao R$ 712,06/mes, edicao recalcula, exclusao funcional | |
