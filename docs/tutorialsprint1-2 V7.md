# Tutorial de Validação Manual — Sprint 1 — Conjunto 2 V7
...
> **CHANGELOG V7 (vs V6) — incorpora 25 melhorias da revisão Arnaldo (06/05/2026):**
>
> **Bugs corrigidos no código (sem mudança no tutorial — só executa pra confirmar):**
> - F03-01 — Badge "Falta" agora distingue **Vencido** (data passada) de **Falta envio** (sem PDF) — antes ambos diziam "Falta".
> - F04-04 — Botão "atualizar esta certidão" individual agora atualiza **APENAS a certidão da linha** (antes acionava busca geral em todas).
> - F04-06 — Validade do PDF prevalece sobre data digitada pelo user. Se IA detecta divergência, **avisa explicitamente** e usa a data real.
> - F04-07 — Validação de magic bytes %PDF antes de salvar arquivo (rejeita HTML de erro/login que scrapers retornam).
> - F04-08 — CRF FGTS agora **persiste o caminho do PDF** (antes salvava só a data de validade, sumia o ícone do olho).
> - F04-01 — Lista de Fontes de Certidões filtra automaticamente por **UF da empresa** (Bio-Hosp/MG não vê mais SEFAZ-PR/SP).
>
> **Mudanças visíveis na UI (validador vai notar):**
> - F01-04 — Campo CNPJ fica **read-only após empresa salva** (CNPJ é chave fiscal). Renomear empresa precisa ser pelo CRUD.
> - F01-07 — Endereço quebrado em **CEP, Logradouro, Número, Complemento, Bairro, Cidade, UF**. CEP popula automaticamente os outros.
> - F01-02 — Inscrição Estadual agora é **obrigatória** (asterisco vermelho). Digite "ISENTO" se a empresa for isenta.
> - F01-08 — Sidebar **lembra preferência** (qual seção estava expandida na última sessão) + botão "Recolher tudo".
> - F02-02 — Cursor "mãozinha" agora aparece em todos elementos clicáveis.
> - F04-02 — Label "Requer autenticação" trocada por mais clara: "Requer credencial para acessar (marque se NÃO for público)".
> - F04-03 — Coluna "Fonte" (Ativa/Inativa) agora aparece na lista de Certidões Automáticas.
> - F04-05 — Tooltips ricos em todos os ícones de ação.
> - F05-01 — Submenu "Responsáveis" renomeado para **"Responsáveis e Representantes"**.
> - F05-02 — Cadastro de Responsável tem campo **Validade do mandato/procuração**.
> - F05-03 — Cadastro de Responsável (especialmente Preposto) tem campos **Documento de Outorga** (descrição + PDF).
> - F13-01 — Sistema agora **rejeita áreas/classes duplicadas** com nome igual + mensagem amigável.
> - F03-03 — Toda tela com resultado de IA tem **checkbox de aceite obrigatório** + log auditável (proteção legal).
>
> **Pedido de fluxo (V7):**
> - F02-01 — Adicionada nota explicativa sobre **por que F02 vem antes de F03/F04 mas precisa de F13 já feito**.
> - F01-03 — Vincular empresa-usuário **não exige mais re-login** (lista atualiza automaticamente).
>
