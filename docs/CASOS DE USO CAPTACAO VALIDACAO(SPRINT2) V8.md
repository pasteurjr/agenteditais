...
<!-- V8 GENERATED — refinamentos para validacao automatizada deterministica em 2026-05-03 -->


**Data:** 2026-04-21 (revisao V8 em 2026-05-03)
**Versao:** 5.1
...
**Novidade V4:** Documentacao das Regras de Negocio (RNs) ja enforcadas no backend em modo warn-only. Conteudo V3 integralmente preservado; foram adicionadas anotacoes `**RNs aplicadas:**` nos UCs afetados e a secao "Regras de Negocio Implementadas (V4)" abaixo.
**Novidade V5:** Cada UC agora inclui secoes de **Fluxos Alternativos (FA)** e **Fluxos de Excecao (FE)**, documentando caminhos alternativos validos e situacoes de erro/excecao. Todo o conteudo V4 permanece integralmente preservado.
**Novidade V8 (2026-05-03):** Refinamentos para validacao automatizada deterministica.
- **UC-CV01:** durante validacao automatizada, [Select: "Fonte"] eh OBRIGATORIAMENTE setado em "PNCP" (evita variabilidade de scrapers de outras fontes), e [Select: "Analise de Score"] eh OBRIGATORIAMENTE "Score Hibrido" (garante ordenacao por score desc na grade resultante — quando `tipoScore !== "nenhum"`, o `defaultSortKey="score" + defaultSortDirection="desc"`).
- **UC-CV02, UC-CV03, UC-CV07:** quando seguidos de UC-CV01 com Score Hibrido, a "primeira linha da grade" passa a referir-se inequivocamente ao "edital com maior score". Os tutoriais visuais foram atualizados para clicar nessa primeira linha que agora representa o melhor candidato. Conteudo funcional permanece o mesmo — apenas explicitando o criterio de selecao.

Conteudo V7 integralmente preservado.
...
**Total de RNs unicas mapeadas para os UCs de CAPTACAO/VALIDACAO (Sprint 2):** 45 RNs (RN-043 a RN-087), distribuidas entre 11 UCs (UC-CV01 a UC-CV04, UC-CV07 a UC-CV13). Destas, 13 sao marcadas `[FALTANTE->V4]` (implementadas nesta versao): RN-078, RN-079, RN-080, RN-081, RN-082, RN-083, RN-084, RN-085, RN-086, RN-087. Os UCs UC-CV05 e UC-CV06 nao possuem RNs mapeadas em `/tmp/rns_CAPTACAO.md`.

| RN | Descricao | UC afetado | Arquivo backend |
|---|---|---|---|
| RN-077 | Termo de busca >=3 caracteres | UC de busca por termo | `backend/app.py` |
| RN-078 | Filtro UF so aceita UFs IBGE validas (27 estados + DF) | UC de busca com filtro UF | `backend/rn_validators.py::validar_uf` |
| RN-082 | Transicao de estado Edital: novo->analisando->participando->proposta_enviada->em_pregao->vencedor/perdedor | UCs de gestao de edital (salvar, analisar, decidir) | `backend/rn_estados.py::EDITAL_TRANSITIONS` |
| RN-080 | Versionamento de decisao GO/NO-GO (toda mudanca em estrategias-editais e logada em AuditoriaLog com snapshot antes/depois) | UC de decisao GO/NO-GO | `backend/rn_audit.py::log_transicao` + `crud_routes._AUDITED_TABLES` |
| RN-083 | Escopo de chat limitado ao edital aberto (impede tool em chat consultar outro edital) | UC de chat IA em validacao | `backend/rn_deepseek.py::validar_scope_edital` |
| RN-084 | Cooldown de 60s entre chamadas DeepSeek por empresa (retorna HTTP 429 se ativado) | UC de analise IA, chat, geracao de proposta | `backend/rn_deepseek.py::check_cooldown` (wired em `/api/chat`) |
| RN-132 | Audit log de invocacoes de ferramentas DeepSeek (tool name, input hash, duracao) | Todos os UCs com tools IA | `backend/rn_audit.py::audited_tool` |
| RN-037 | Audit log universal (editais, propostas, estrategias sao logadas em AuditoriaLog) | UCs de CRUD de edital, proposta, estrategia | `backend/rn_audit.py::log_transicao` |
...
- `Relatorio Completo`: gera markdown/HTML e abre a trilha de consolidacao da busca.
...
- `Participar (GO)`, `Acompanhar (Em Avaliacao)`, `Rejeitar (NO-GO)`: mudam o estado visual e abrem o bloco de justificativa.
...
- `Verificar Certidoes`: reexecuta busca de certidoes e tenta recarregar a completude documental.
- `Analisar Riscos do Edital` e `Reanalisar...`: chamam pipeline unificado de riscos.
- `Rebuscar Atas`, `Buscar Vencedores e Precos`, `Atualizar`: alimentam historico competitivo.
- `Analisar Mercado do Orgao` e `Reanalisar Mercado`: chamam `/api/editais/{id}/analisar-mercado`.
- `Gerar Resumo`, `Regerar Resumo`, `Perguntar`, `Requisitos Tecnicos`, `Classificar Edital`: usam `/api/chat` com sessao de pagina.
...
2. Na aba `Documentos` da `ValidacaoPage`, o botao `Verificar Certidoes` envia `POST /api/empresa-certidoes/buscar-stream` com `empresa_id: edital.id`, o que aparenta misturar identidade de empresa com identidade de edital.
...
3. Usuario ajusta filtros de [Select: "UF"], [Select: "Fonte"], [Select: "Modalidade"], [Select: "Origem"], [Campo: "NCM"] e [Campo: "Periodo (De/Ate)"]. **V8 — para validacao automatizada deterministica, [Select: "Fonte"] = "PNCP" eh OBRIGATORIO** (evita variabilidade de scrapers; PNCP eh a unica fonte oficial nacional 100% disponivel via API). [ref: Passo 3]
4. Usuario define o modo de score no [Select: "Analise de Score"]: `Sem Score`, `Score Rapido`, `Score Hibrido` ou `Score Profundo`. **V8 — para validacao automatizada, "Score Hibrido" eh OBRIGATORIO**: garante calculo combinado (local + DeepSeek nos top N) e dispara ordenacao automatica da grade por score desc (`defaultSortKey="score" + defaultSortDirection="desc"` em `CaptacaoPage.tsx:2342`). UCs CV02/CV03/CV07 a partir desse ponto trabalham com a primeira linha da grade = edital de maior pontuacao, escolha deterministica e auditavel. [ref: Passo 4]
...
8. Os [Card: "StatCard"] superiores (prazo 2, 5, 10, 20 dias) e a [Tabela: "Resultados"] sao atualizados com os novos dados. **V8 — Quando `tipoScore !== "nenhum"` a tabela aparece ordenada por score desc.** [ref: Passo 8]
