# Tutorial de Validação Manual — Sprint 4 — Conjunto 2 V8
# Empresa: Bio-Hosp Equipamentos Hospitalares Ltda.

**Data:** 18/05/2026 (V8)
**Dados:** dadosrecursimp-2 V8.md (continuidade do tutorialsprint2-2 V8.md)
...
> **CHANGELOG V8**
>
> Conjunto 2 V8 — encadeado com `tutorialsprint2-2 V8.md` (mesma empresa **Bio-Hosp Equipamentos Hospitalares Ltda.**). Estrutura de UCs idêntica ao molde validado (`tutorialsprint4-1.md`); identidade do conjunto adaptada (empresa / usuário / CNPJ / produtos). Reflete o estado pós-correções das 22 observações Arnaldo (Sprint 1) e melhorias de UX (cursor mãozinha, sidebar persistente, badge de 4 estados, endereço estruturado, máscaras de campo).
...
**Pre-condicoes:**
- Usuario autenticado no sistema (validaarnaldo@valida.com.br, empresa: Bio-Hosp)
...
await expect(page.locator('text=Bio-Hosp')).toBeVisible();
await expect(page.locator('text=33.014.556/0001-96')).toBeVisible();
...
| Impugnante | Bio-Hosp Equipamentos Hospitalares Ltda. |
| CNPJ Impugnante | 33.014.556/0001-96 |
...
| 4 | Verificar qualificacao | `text=Bio-Hosp` | — | Razao social no texto |
| 5 | Verificar CNPJ | `text=33.014.556/0001-96` | — | CNPJ presente |
...
**Pagina:** `ImpugnacaoPage` — rota `/app/impugnacao` (aba Peticoes)
**Pre-condicoes:**
- Usuario autenticado (validaarnaldo@valida.com.br, empresa: Bio-Hosp)
...
| Recorrente | Bio-Hosp Equipamentos Hospitalares Ltda. |
| CNPJ Recorrente | 33.014.556/0001-96 |
| Representante Legal | Diego Ricardo Munoz — Socio-Administrador |
...
| 3 | Verificar cabecalho | `text=PREGOEIRO` ou `text=COMISSAO` | — | Cabecalho presente |
| 4 | Verificar recorrente | `text=Bio-Hosp` | — | Razao social no texto |
| 5 | Verificar CNPJ | `text=33.014.556/0001-96` | — | CNPJ presente |
| 6 | Verificar representante | `text=Diego Ricardo Munoz` | — | Nome presente |
| 7 | Verificar secao Dos Fatos | `text=Dos Fatos` | — | Secao presente |
...
**Pre-condicoes:**
- Usuario autenticado (validaarnaldo@valida.com.br, empresa: Bio-Hosp)
- Edital de "desfibrilador externo automatico" (segundo edital) selecionado
- Cenario: Bio-Hosp e a vencedora e precisa se defender de recurso
...
// Selecionar edital de "desfibrilador externo automatico"
await page.click('select, [role="combobox"]');
await page.click('text=desfibrilador');
...
| Campo | Valor |
|---|---|
| Edital | Edital de "desfibrilador externo automatico" (segundo edital salvo) |
| Tipo de Laudo | Contra-Razao de Recurso |
| Contra-Arrazoante | Bio-Hosp Equipamentos Hospitalares Ltda. |
| CNPJ | 33.014.556/0001-96 |
| Recorrente (adversario) | BioEquip Distribuidora de Equipamentos Medicos S.A. |
| Motivo do Recurso do Adversario | Alega que preco da Bio-Hosp e inexequivel |
...
| 4 | Fabricante Nihon Kohden fornece nota de credito por volume que justifica desconto | Documentacao comprobatoria do fabricante |
...
| Do Pedido | Manutencao da adjudicacao em favor da Bio-Hosp |
| Fechamento | Local, data, assinatura |
...
|---|------|----------|------|--------|
| 1 | Clicar em Recursos na sidebar | `button:has-text("Recursos")` | — | URL contem `/app/recursos` |
| 2 | Selecionar edital | `select, [role="combobox"]` | Edital "desfibrilador externo automatico" | Edital selecionado |
...
| 6 | Verificar contra-arrazoante | `text=Bio-Hosp` | — | Razao social presente |
| 7 | Verificar CNPJ | `text=33.014.556/0001-96` | — | CNPJ presente |
...
**Pre-condicoes:**
- Usuario autenticado (validaarnaldo@valida.com.br, empresa: Bio-Hosp)
- Editais de "monitor multiparametrico" e "desfibrilador externo automatico" com propostas finalizadas
...
| Campo | Valor |
|---|---|
| Edital | Edital de "desfibrilador externo automatico" (segundo edital) |
| Resultado | Perdido |
...
| 10 | Selecionar edital ultrassom | `select, [role="combobox"]` | Edital "desfibrilador externo automatico" | Edital selecionado |
...
> **Nota para execucao manual:** Registrar resultado "Ganho" para o edital de monitor multiparametrico e "Perdido" para o de desfibrilador externo automatico. Verificar que os badges aparecem nas linhas correspondentes.
...
| Canal | Ativo |
|---|---|
| Email (licitacoes@biohosp.com.br) | Sim |
...
- Usuario: `validaarnaldo@valida.com.br` / Senha: `123456`
- Perfil: Superusuario
- Empresa: Bio-Hosp Equipamentos Hospitalares Ltda.
