# Tutorial de Validação Manual — Sprint 3 — Conjunto 2 V8
# Empresa: Bio-Hosp Equipamentos Hospitalares Ltda.

**Data:** 18/05/2026 (V8)
**Dados:** dadosprecprop-2 V8.md (continuidade do tutorialsprint2-2 V8.md)
**Referência:** CASOS DE USO PRECIFICACAO E PROPOSTA V2.md
**Pré-requisito:** `tutorialsprint2-2 V8.md` executado com sucesso (empresa Bio-Hosp + portfólio + parametrização + edital captado/validado)
...
- Produtos do portfolio cadastrados na Sprint 1 (Monitor BSM-3000 e Desfibrilador HeartStart FRx)
...
|---|---|---|---|
| Monitor multiparametrico 12 parametros | Monitor Multiparametro Nihon Kohden BSM-3000 | >= 80% | Vinculado automaticamente |
...
**O que voce vai ver na tela:** O item 1 (Monitor) deve ter um badge verde "Vinculado" com o nome do produto `Monitor Multiparametro Nihon Kohden BSM-3000`. Os itens 3, 4 e 5 devem ter badge cinza "Sem match".
...
**O que o validador deve conferir:**
- Item 1 (Monitor) vinculado automaticamente ao `Monitor Multiparametro Nihon Kohden BSM-3000` com match >= 80%
...
- Item 1 nao e vinculado ao Monitor BSM-3000
...
### Passo 1 — Informar custos ERP do Monitor BSM-3000

**O que fazer:** Na aba "Custos e Precos", localize a secao de custos para o produto vinculado. Preencha os custos ERP do `Monitor Multiparametro Nihon Kohden BSM-3000`:

| Campo | Valor |
|---|---|
| Produto | `Monitor Multiparametro Nihon Kohden BSM-3000` |
| SKU | `NK-BSM3000-BR` |
...
### Passo 1 — Calcular Preco Base do Monitor BSM-3000 (Custo + Markup)

**O que fazer:** Selecione o modo "Custo + Markup" (radio button). O sistema calcula automaticamente o Preco Base para o `Monitor Multiparametro Nihon Kohden BSM-3000`:
...
**O que o validador deve conferir:**
- Monitor BSM-3000: Preco Base calculado = `R$ 22.486,40` (Custo + Markup 28%)
...
|---|---|---|---|
| Monitor BSM-3000 | `R$ 22.486,40` | `-18%` | `R$ 18.438,85` |
...
|---|---|---|---|
| Monitor BSM-3000 | `R$ 18.500,00` | `R$ 17.800,00` | `3,8%` |
...
|---|---|---|---|
| Monitor BSM-3000 | `R$ 17.800,00` | `R$ 16.500,00` | `17,5%` |
...
**O que fazer:** Na aba "Historico", selecione o produto `Monitor Multiparametro Nihon Kohden BSM-3000` e o periodo `Ultimos 24 meses`. Clique em `Filtrar`.
...
| 2025-06-10 | Hospital Estadual RJ | RJ | R$ 18.900,00 | R$ 15.900,00 | Bio-Hosp |
...
- Produto do portfolio cadastrado (Monitor BSM-3000)
...
| Campo | Valor |
|---|---|
| Equipamento | `Monitor Multiparametro Nihon Kohden BSM-3000` |
| Numero de serie | `NK-BSM3000-2026-001` |
...
**O que o validador deve conferir:**
- Comodato cadastrado com equipamento `Monitor BSM-3000`, serie `NK-BSM3000-2026-001`, prazo `60 meses`
...
**O que fazer:** Analise as sugestoes geradas pelo pipeline para o `Monitor BSM-3000`:
...
| Secao | Conteudo |
|---|---|
| Cabecalho | Empresa (Bio-Hosp), edital, lote, data de geracao |
...
- Produto `Monitor Multiparametro Nihon Kohden BSM-3000` com especificacoes cadastradas no portfolio
...
| Produto ofertado | Portfolio | `Monitor Multiparametro Nihon Kohden BSM-3000` |
| Fabricante | Portfolio | `Nihon Kohden` |
| Modelo | Portfolio | `BSM-3000` |
...
- Fabricante `Nihon Kohden`, registro ANVISA `80262090001`
...
**O que fazer:** No card "Auditoria ANVISA", verifique que o produto `Monitor Multiparametro Nihon Kohden BSM-3000` esta selecionado com registro `80262090001`. Clique em `Verificar Registros`.
...
| Secao | Conteudo |
|---|---|
| Capa | Empresa (Bio-Hosp), edital, lote, data |
...
| UC-P10 | Comodato cadastrado serie NK-BSM3000-2026-001, amortizacao R$ 712,06/mes, edicao recalcula, exclusao funcional | |
