# Tutorial de Validação Manual — Sprint 2 — Conjunto 2 V6
# Empresa: Bio-Hosp Equipamentos Hospitalares Ltda.

**Data:** 06/05/2026 (V6)
**Dados:** dadoscapval-2 V6.md (continuidade do tutorialsprint1-2 V6.md)
**Referência:** CASOS DE USO CAPTACAO VALIDACAO(SPRINT2) V8.md
**Pré-requisito:** `tutorialsprint1-2 V6.md` executado com sucesso (empresa Bio-Hosp + portfólio + parametrização cadastrados)
...
**Antes de iniciar esta Sprint 2, você deve ter concluído o tutorial `tutorialsprint1-2 V6.md`.**

A Sprint 2 **continua** os dados criados na Sprint 1 V6:

| Item | Cadastrado em (UC) | Sprint 2 usa em |
|---|---|---|
| Empresa **Bio-Hosp Equipamentos Hospitalares Ltda.** (CNPJ `33.014.556/0001-96`, UF MG) | UC-F01 | Filtro de empresa em todas as buscas e gravações |
| Hierarquia **Equipamentos Médico-Hospitalares → Monitoração / Monitorização Multiparamétrica → Monitor Multiparâmetro / Oxímetro de Pulso** | UC-F13 | Filtros Área/Classe/Subclasse na busca (UC-CV01) |
| **Máscara de campos** das subclasses (`campos_mascara`) | UC-F13 PARTE 4 | Reaproveitada pelo cálculo de score (UC-CV01/CV08) |
| Produto **Monitor Multiparâmetro Nihon Kohden BSM-3000** com 8 specs corretas | UC-F07 + UC-F08 | Match de produto vs edital e cálculo de aderência |
| **Fontes de Certidões** (PGFN, SEFAZ-MG, Pref. Contagem) | UC-F04 | Validação automática de habilitação ao salvar edital (UC-CV03) |
...
Se você seguiu o `tutorialsprint1-2 V6.md` corretamente, esses dados já estão cadastrados.
...
>
> Atualização **completa de continuidade** com `tutorialsprint1-2 V6.md`:
> - **Empresa alvo**: `Bio-Hosp Equipamentos Hospitalares Ltda.` (CNPJ `33.014.556/0001-96`, UF MG)
> - **Login**: `validaarnaldo@valida.com.br` (super, mesmo da Sprint 1)
...
>   - V6: termos de **"monitor multiparâmetro"**, **"oxímetro pulso"**, **"equipamento médico hospitalar"**, alinhados aos produtos da Bio-Hosp.
> - **NCM** atualizado para `9018.19.90` (NCM dos monitores cadastrados em UC-F13 da Sprint 1 V6, conforme dados-2 V6.md).
> - **UF padrão** `MG` (estado da Bio-Hosp), com algumas buscas em `SP` e `RJ` para validar filtros multi-UF.
> - **Filtro Área/Classe/Subclasse** preenchido com a hierarquia real cadastrada em UC-F13 V6:
>   - Área: `Equipamentos Médico-Hospitalares`
>   - Classe: `Monitoração` ou `Monitorização Multiparamétrica`
>   - Subclasse: `Monitor Multiparâmetro` / `Monitor Multiparamétrico` / `Oxímetro de Pulso`
> - **Score Profundo** valida o match com o produto `Monitor Multiparâmetro Nihon Kohden BSM-3000` cadastrado em UC-F07.
> - **Edital alvo principal**: edital fictício/real do **VERE 0000031/2026** ou qualquer edital de monitoração multiparamétrica do PNCP que apareça nos resultados.
...
| Campo | Valor |
|---|---|
| Usuário (Conjunto 2) | validaarnaldo@valida.com.br |
| Senha | 123456 |
| Perfil | Superusuário |
| Empresa alvo | Bio-Hosp Equipamentos Hospitalares Ltda. |

### Pré-requisito — Associar validaarnaldo à empresa Bio-Hosp
...
2. Ao ver a tela de seleção de empresa, selecionar "Bio-Hosp Equipamentos Hospitalares Ltda." (se já existir do Sprint 1)
3. Se a Bio-Hosp não existir ainda, criá-la via Menu Empresa conforme UC-F01 do Sprint 1
...
> Esses menus não aparecem para usuários normais (super=False). O tutorial a seguir assume que a empresa Bio-Hosp já está associada a validaarnaldo e que os dados do Sprint 1 (empresa, portfólio e parametrização) já estão cadastrados.
...
- Certifique-se de estar logado com `validaarnaldo@valida.com.br` e com a empresa Bio-Hosp ativa.
- Os dados do Sprint 1 V6 (portfólio Bio-Hosp + parametrização + máscara de campos) devem estar cadastrados — o cálculo de score depende desses dados.
...
### Passo 2 — Executar a Busca 1: "monitor multiparametrico"
...
📌 **Atenção:** O campo NCM (Nomenclatura Comum do Mercosul) é um código numérico que classifica mercadorias. Digite exatamente `9018.19.90` — este é o código para monitores multiparamétricos e similares. O campo pode ter máscara automática para os pontos.
...
- Você deve ter executado pelo menos a Busca 1 ("monitor multiparametrico") do UC-CV01 e ter resultados visíveis na tela.
...
**O que fazer:** Na lista de resultados da Busca 1 ("monitor multiparametrico"), localize o primeiro edital que tenha um score igual ou superior a 40%. Clique sobre ele (no título ou em um botão "Ver detalhes").
...
📌 **Atenção:** O produto compatível esperado é `Monitor Multiparâmetro Nihon Kohden BSM-3000`. Verifique se este nome (ou algo muito similar) aparece na seção de compatibilidade do painel.

**O que acontece depois:** As informações ficam visíveis no painel para consulta.

✅ **Correto se:** Todas as informações listadas acima estão visíveis no painel. O produto `Monitor Multiparâmetro Nihon Kohden BSM-3000` aparece como match.
...
- O produto `Monitor Multiparâmetro Nihon Kohden BSM-3000` apareceu como match no primeiro edital da Busca 1
...
**O que fazer:** Execute a Busca 1 ("monitor multiparametrico") se a lista não estiver visível. Na lista de resultados, localize um edital com bom score. Clique no botão "Salvar" (ou ícone de disquete/favorito) que aparece ao lado desse edital específico.
...
**O que fazer:** Execute a Busca 4 ("monitoracao paciente", UF `MG`, Sem Score). Na lista de resultados, localize e clique no botão "Salvar Todos" (pode estar no topo da lista ou como uma opção de menu).
...
### Passo 1 — Definir Estratégia 1: Estratégico para Monitor Multiparâmetro

**O que fazer:** Localize um edital salvo da Busca 1 ("monitor multiparametrico"). Abra seu painel ou acesse a opção de "Definir Estratégia" (pode ser um botão, um dropdown, ou uma aba no painel lateral).
...
**O que fazer:** Execute a Busca 1 ("monitor multiparametrico", Todas UFs, PNCP, Score Rápido, encerrados=Não). Com os resultados visíveis, localize e clique no botão "Exportar CSV" (pode estar no topo ou no rodapé da lista de resultados, ou em um menu de ações).
...
### Passo 1 — Criar o Monitoramento 1: Monitores multiparamétricos
...
| Campo | Valor |
|---|---|
| Termo de busca | `monitor multiparametrico monitoração` |
| NCM | `9018.19.90` |
| UFs | `SP, MG, PR, RS` |
| Fonte | `PNCP` |
...
**O que fazer:** Na lista de monitoramentos, clique em "Editar" no Monitoramento 1 ("monitor multiparametrico monitoração"). Altere o score mínimo de `50` para `60` e salve.
...
- Ao final, restam 2 monitoramentos ativos:
  - Mon 1: "monitor multiparametrico monitoração", score mínimo `60` (atualizado), 4 UFs, 6h
...
**O que fazer:** Limpe o filtro de busca. Localize um edital da Busca 1 ("monitor multiparametrico") e clique nele para abrir a tela de análise detalhada.
...
- Se necessário, navegue para Validação e clique em um edital de "monitor multiparametrico".
...
- Selecione o edital que recebeu decisão GO no UC-CV08 (o de "monitor multiparametrico").
...
| # | Item esperado |
|---|---|
| 1 | Monitor multiparamétrico de paciente adulto |
| 2 | Sensor de SpO2 reutilizável adulto |
| 3 | Cabo paciente ECG 5 vias |
| 4 | Manguito PNI adulto reutilizável |
| 5 | Sensor de oxímetro de pulso descartável |
| 6 | Cabo de extensão SpO2 |
| 7 | Eletrodo descartável adulto |
...
|---|---|---|
| Lote 1 | Monitoração — Equipamento Principal | Itens 1–4 (monitor multiparamétrico, sensores SpO2, cabo ECG, manguito) |
| Lote 2 | Monitoração — Acessórios SpO2 | Itens 5–6 (sensor descartável, cabo extensão) |
...
**O que fazer:** Localize o item 8 (ponteiras / acessórios diversos) no Lote 3 (Material de Consumo). Arraste-o para o Lote 1 (Monitoração — Equipamento Principal) ou use uma opção de "Mover para..." e selecione "Lote 1".
...
- Resultado final: 2 lotes — Lote 1 (Monitoração — Equipamento Principal, 6 itens) e Lote 2 (Monitoração — Acessórios SpO2, 2 itens)
...
> **O que este caso de uso faz:** Cada edital exige uma série de documentos da empresa para habilitação — certidões fiscais, alvarás, registros técnicos, etc. Este caso de uso usa inteligência artificial para identificar quais documentos o edital exige, verifica quais a empresa já possui (cadastrados no Sprint 1) e sinaliza o que está faltando ou vencido. Para uma empresa de equipamentos médico-hospitalares (como a Bio-Hosp), documentos como AFE ANVISA, Registro de Produto e Certificado de Boas Práticas de Fabricação são obrigatórios — o sistema precisa identificar isso automaticamente. Pense nisso como um checklist de embarque: antes de viajar, você confere se tem passaporte, visto e vacinas em dia.
...
- Selecione o mesmo edital do UC-CV09 (o de "monitor multiparametrico" com decisão GO).
...
**O que fazer:** Clique no botão "Analisar Riscos" (ou "Identificar Riscos", "Avaliar Riscos", ou similar). A IA vai processar o edital e identificar os riscos específicos para a participação da Bio-Hosp.
...
📌 **Atenção:** Os riscos "Especificação de marca exclusiva" (Crítico) e "Registro ANVISA do produto" (Alto) são os mais importantes para equipamentos médico-hospitalares. Se o edital exige uma marca/modelo específico, a Bio-Hosp pode ser eliminada — o sistema deve sinalizar isso como risco crítico.
...
| Histórico Interno | Participações anteriores da Bio-Hosp com esse órgão (pode estar vazio se for o primeiro contato) |
| Análise IA | Texto consolidando as informações acima em uma análise de mercado |

📌 **Atenção:** A seção "Compras Similares" deve mostrar compras anteriores de equipamento médico-hospitalar (não de outros materiais). A seção "Histórico Interno" pode estar vazia se a Bio-Hosp nunca participou de licitação com esse órgão — isso é normal.
...
- Selecione o mesmo edital dos UCs anteriores (o de "monitor multiparametrico" com decisão GO).
- Navegue até a tela de detalhes deste edital.
...
> **Dica final:** Faça os UCs na ordem apresentada neste tutorial. Os UCs de Captação (CV01–CV06) devem ser feitos antes dos de Validação (CV07–CV13), pois os editais precisam ser salvos na Captação antes de aparecerem na Validação. Se precisar recomeçar, limpe os dados da Bio-Hosp antes de começar novamente.
